...
use cms_core::api::knowledge::{Create, DocumentUpdate, DraftUpdate, VisibilityUpdate};
use serde_json::json;

pub async fn run(action: &str, arguments: &[String]) -> Result<(), String> {
    match (action, arguments) {
        ("list", []) => {
            let page = cms_core::api::knowledge::list(None)
                .await
                .map_err(|error| error.to_string())?;
            print_json(&page)
        }
        ("list", [cursor]) => {
            let page = cms_core::api::knowledge::list(Some(cursor.to_owned()))
                .await
                .map_err(|error| error.to_string())?;
            print_json(&page)
        }
        ("get", [id]) => {
            let article = cms_core::api::knowledge::get(id)
                .await
                .map_err(|error| error.to_string())?;
            print_json(&article)
        }
        ("create", [input]) => {
            ...
            let article = cms_core::api::knowledge::create(&input)
                .await
                .map_err(|error| error.to_string())?;
            print_json(&article)
        }
        ("update-draft", [id, input]) => {
            ...
            let article = cms_core::api::knowledge::update_draft(id, &input)
                .await
                .map_err(|error| error.to_string())?;
            print_json(&article)
        }
        ("update-documents", [id, input]) => {
            ...
            let article = cms_core::api::knowledge::update_documents(id, &input)
                .await
                .map_err(|error| error.to_string())?;
            print_json(&article)
        }
        ("visibility", [id, input]) => {
            ...
            let article = cms_core::api::knowledge::set_visibility(id, &input)
                .await
                .map_err(|error| error.to_string())?;
            print_json(&article)
        }
        ("delete", [id, expected_hash]) => {
            cms_core::api::knowledge::delete(id, expected_hash)
                .await
                .map_err(|error| error.to_string())?;
