	...
	<div class="status-content">
		{#if service !== null}
			<div class="summary">
				...
				<time datetime={service.updatedAt}>{formatTime(service.updatedAt)}</time>
			</div>
			<div
...
<style>
	.service-status-panel { display: flex; flex-direction: column; width: 100%; min-width: 0; box-sizing: border-box; padding: 0.75rem; border: 1px solid var(--color-border); border-radius: var(--radius-lg); background: var(--color-background); box-shadow: var(--shadow-xs); transition: transform var(--duration-slow) cubic-bezier(0.16, 1, 0.3, 1), border-color var(--duration-slow) cubic-bezier(0.16, 1, 0.3, 1), box-shadow var(--duration-slow) cubic-bezier(0.16, 1, 0.3, 1); }
	.status-content { display: flex; flex: 1; flex-direction: column; justify-content: center; min-width: 0; padding-top: 0.5rem; }
	.status-details { min-width: 0; }
	...
	.health-bar { flex-shrink: 0; height: 0.55rem; margin-top: 0.65rem; overflow: hidden; border-radius: var(--radius-full); background: var(--color-muted); }
	...
	p.message { margin: 0; min-height: 4.7rem; display: grid; place-items: center; color: var(--color-muted-foreground); font-size: 0.68rem; text-align: center; }
