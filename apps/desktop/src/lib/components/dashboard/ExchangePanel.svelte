<script lang="ts">
	...
	function findRate(code: string): ExchangeRate | null {
	...
	function calculatePair(baseCode: string): { code: string; value: number; changePercent: number } | null {
		const base = findRate(baseCode);
		const cny = findRate("CNY");
	...
	let rows = $derived(pairs.map(calculatePair).filter((value): value is NonNullable<typeof value> => value !== null));
	let date = $derived.by(() => {
		const cny = findRate("CNY");
...
<style>
	.exchange-panel { width: 100%; min-width: 0; box-sizing: border-box; padding: 0.75rem; border: 1px solid var(--color-border); border-radius: var(--radius-lg); background: var(--color-background); box-shadow: var(--shadow-xs); }
	header,
	header span,
	.rates > div,
	...
	.rates { display: grid; gap: 0.05rem; margin-top: 0.5rem; }
	.rates > div { min-width: 0; gap: 0.6rem; padding: 0.35rem 0; border-bottom: 1px solid color-mix(in srgb, var(--color-border) 65%, transparent); }
