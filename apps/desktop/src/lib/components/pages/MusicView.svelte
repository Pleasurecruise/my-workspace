...
<section class="music" class:player-visible={playerVisible} aria-label={`${providerName} ${collectionName}`}>
	<header class="page-header">
		<div><h1>{playerVisible ? "Now playing" : collectionName}</h1><p class="page-description">{providerName} · {tracks.length} songs</p></div>
		{#if !playerVisible}
...
<style>
	...
	.empty { display: grid; min-height: 20rem; place-items: center; align-content: center; gap: 0.65rem; color: var(--color-muted-foreground); text-align: center; }
	...
	:global(.page-content[data-stacked="true"]) .album { display: none; }
	:global(.page-content[data-stacked="true"]) .track-list > button { grid-template-columns: 1.2rem 2.6rem minmax(0, 1fr) 2.5rem; }
	@media (prefers-reduced-motion: reduce) { .record.spinning, .subtitle p { animation: none; } }
</style>
