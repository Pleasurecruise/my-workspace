...
<section class="home" aria-label="Memo feed">
	<header class="page-header">
		<div><h1>{display === "archived" ? "Archive" : display === "favorites" ? "Favorites" : "Memos"}</h1><p class="page-description">{display === "archived" ? "Archived memos, ready to restore." : display === "favorites" ? "Memos and posts you want to keep." : "Capture thoughts and everyday notes."}</p></div>
	</header>
	{@render tagStatus?.()}
	{#if display === "active"}
				...
				<footer>
					...
					{:else}
						{#if confirmingDelete === memo.id}
							...
							<Button variant="outline" size="sm" class="rounded-md border-error/35 bg-transparent font-normal text-error hover:border-error/70 hover:bg-transparent focus-visible:ring-error" disabled={deleting} onclick={() => remove(memo)}>
...
<style>
	...
	.collection-group > header h2 { margin: 0; }
	...
	:global(.page-content[data-stacked="true"]) .x-import { grid-template-columns: minmax(0, 1fr) auto; }
	:global(.page-content[data-stacked="true"]) .x-import :global(input) { grid-column: 1 / -1; }
	:global(.page-content[data-stacked="true"]) .shortcut { display: none; }
